...
class ezdata:
    ...
    def make(self,test=None):

        if hasattr(self,"synsets"):
            if test is not None:
                self.gen_test(test)
                np.savez(self.params["name"]+"_test.npz",X=self.X_test,y=self.y_test,synsets=self.synsets)
                print("[X] Test set has been save into: " + self.params["name"]+"_test.npz")
            np.savez(self.params["name"]+"_train.npz",X=self.X,y=self.y,synsets=self.synsets)
            print("[X] Training set has been save into: " + self.params["name"]+"_train.npz")
        else:
            if test is not None:
                self.gen_test(test)
                np.savez(self.params["name"]+"_test.npz",X=self.X_test,y=self.y_test,synsets=self.synsets)
                print("[X] Test set has been save into: " + self.params["name"]+"_test.npz")
            np.savez(self.params["name"]+"_train.npz",X=self.X,y=self.y)
            print("[X] Training set has been save into: " + self.params["name"]+"_train.npz")
