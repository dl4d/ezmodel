...
vgg16 = VGG16(include_top=False, weights='imagenet', input_shape=train.X.shape[1:], pooling=None)


input = Input(shape=vgg16.output_shape[1:])
x = UpSampling2D(size=(2,2)) (input)
x = Conv2DTranspose(filters=512,kernel_size=(3,3),padding="same",activation="relu") (x)
x = Conv2DTranspose(filters=512,kernel_size=(3,3),padding="same",activation="relu") (x)
x = Conv2DTranspose(filters=512,kernel_size=(3,3),padding="same",activation="relu") (x)
...
x = Conv2DTranspose(filters=64,kernel_size=(3,3),padding="same",activation="relu") (x)
decoder = Model(input=input,output=x)
